...
// SoftwareSerial Serial1(2, 3);      // RX, TX for ESP8266
...
//  sendToWifi("AT+CWLAP",responseTime,DEBUG); // get available networks
//  sendToWifi("AT+CWJAP=\"JJS1705\",\"23567823657823658263856285\"",responseTime,DEBUG); // connect to network JJS..
  sendToWifi("AT+CWJAP=\"HUAWEI_ALE-L21_A82C\",\"qwertyuiop\"",responseTime,DEBUG); // connect to network JJS..
...
String sendToWifi(String command, const int timeout, boolean debug){
  String response = "";
  Serial.println("sendToWifi In = " +command);
  ...
  if(debug)
  {
    Serial.println("SentToWifi Out = " + response);
  }
  return response;
}
